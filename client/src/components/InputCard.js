...
const useStyles = makeStyles(() => ({
  card: {
    margin: '1.6px 8px 0.72px 8px',
    wordWrap: 'break-word',
    padding: (props) =>
      props.type === 'list' ? '4px 12px 4px 12px' : '8px 8px 28px 16px',
    boxShadow: (props) =>
      props.type === 'list'
  ...
  listBackground: {
    ...
    marginLeft: (props) => (props.marginLeft ? '8px' : '0px'),
    paddingTop: (props) => (props.type === 'list' ? '8px' : 'inherit'),
    borderRadius: '4px',
  },
  width: (props) => ({
    width: props.width,
